...
(() => {
  ...
  async function loadCases() {
    ...
    list.forEach((c) => {
      ...
      div.innerHTML = `<div class="top"><div><div class="nm">${esc(c.verifiedName || '—')}</div><div class="meta">${esc(c.docType || '-')} · Nr.: ${esc(c.docNumber || '-')}<br>Nummer: ${esc(c.code || '-')} · Prüfer: ${esc(c.agentName || '-')} · ${esc(date)}${c.rejectReason ? '<br>Grund: ' + esc(c.rejectReason) : ''}</div></div>${pill}</div><div class="thumbs">${thumbs}</div>`;
